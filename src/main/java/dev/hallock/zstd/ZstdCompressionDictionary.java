...
/**
 * A digested compression dictionary, ready to start compression operations without startup delay.
 *
...
public final class ZstdCompressionDictionary implements AutoCloseable {
  ...
  // (context references << 1) | open bit; ZSTD_freeCDict runs exactly when the state reaches 0.
  private final AtomicInteger state = new AtomicInteger(1);

  ZstdCompressionDictionary(MemorySegment dict, int compressionLevel) {
    this(dict, dict.byteSize(), compressionLevel);
  }

  ZstdCompressionDictionary(MemorySegment dict, long dictSize, int compressionLevel) {
    ...
    MemorySegment cdict = ZSTD_h.ZSTD_createCDict(dict, dictSize, compressionLevel);
    if (cdict.address() == 0) {
      throw new OutOfMemoryError("Failed to allocate native compression dictionary");
    }
    this.cdict = cdict;
  ...
  /** Takes a context reference and returns the native CDict pointer. */
  MemorySegment acquire() {
    ...
    throw new IllegalStateException("Compression dictionary is closed");
  }

  /** Drops a context reference; frees the native CDict when the state reaches 0. */
  void release() {
  ...
  private void freeNative() {
    long result = ZSTD_h.ZSTD_freeCDict(cdict);
  ...
  /**
   * Returns the compression level used to train/compile this dictionary.
   *
   ...
   */
  public long sizeOf() {
    MemorySegment dict = acquire();
    try {
      return ZSTD_h.ZSTD_sizeof_CDict(dict);
    } finally {
      release();
    }
  }
  ...
  public int dictId() {
    MemorySegment dict = acquire();
    try {
      return ZSTD_h.ZSTD_getDictID_fromCDict(dict);
    } finally {
      release();
    }
  }
